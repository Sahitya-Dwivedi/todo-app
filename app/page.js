...
const home = () => {
  ...
  let Check = (e) => {
    if (e.target.checked) {
      todoTask.current.style.textDecoration = "line-through"
    } else {
      todoTask.current.style.textDecoration = "none"
    }
  }
            ...
            todos.map((todo) => {
              return (
                <div key={uuidv4()} className='flex items-center gap-8'>
                  <div className='flex'>
                    <input type="checkbox" name="checkbox" onChange={Check} className="mt-4 mx-4" />
                    <div className="todo w-[95%] px-2 py-1  mt-4 text-center" ref={todoTask}>{todo}</div>
                  </div>
