  ...
  if (todos.length > 0) {
    renderTask = todos.map((todo) => {
      return ((showFinished || !todo.isCompleted) &&
        <div key={uuidv4()} className='flex items-center w-[100%] justify-between'>
          <div className='flex text-wrap'>
            ...
            <div className='flex flex-col'>
              <div className={todo.isCompleted ? "todo w-[100%] px-2   mt-4 line-through font-bold italic" : "todo w-[95%] px-2   mt-4  font-bold italic "}>{todo.todo}</div>
              <ul className='flex gap-2 items-center justify-center'>
                <li className='text-gray-500 text-sm pl-2'>{todo.description}</li>
                <li className='text-gray-500 text-sm list-disc list-inside'>{todo.DueDate}</li>
              </ul>
            </div>
          </div>
          <div className='flex gap-2'>
            <button className="edit mt-4 bg-blue-500 px-2 py-1 rounded-xl" onClick={editTodo} value={todo.todo}>Edit</button>
            <button className="delete mt-4 bg-blue-500 px-2 py-1 rounded-xl mr-2" onClick={deleteTodo} value={todo.todo}>Delete</button>
          </div>
        </div>
      )
    }
    ...
    <>
      <div className="main w-1/2 min-w-60 h-[90vh]  bg-[#eff1f2] mx-auto mt-12 rounded-2xl border-2 border-blue-400 text-black">
        ...
        <div className="input flex gap-2">
          <input type="text" name="your todos" className="w-1/3 border-2 border-black ml-4 px-2 text-xl rounded-md " placeholder="Enter Your Todo." value={todo} onChange={InputChange} onKeyDown={(e) => e.key == "Enter" ? addTodo() : null} ref={InpRef} autoFocus />

          <div className='dueDate relative w-6 h-6 overflow-hidden'>
            <input type="date" name="dueDate" className="dueDate absolute top-0 left-0 opacity-0 z-10 w-6 h-6" title='set due date' onChange={dueDate} />
            <FaCalendarDays className='w-6 h-6 absolute top-0 left-0' title='set due date' />
          </div>

          <button className="add text-xl bg-blue-500 px-2 rounded-xl border-black border-2" onClick={addTodo} ref={add} >Add</button>
        </div>

        <input type="text" name="description" id="description" className='w-1/2 border-2 border-gray-500 mx-4 my-2 px-2 text-xl rounded-md' placeholder='Enter description(optional)' onChange={(e) => setDescription(e.target.value)} />
